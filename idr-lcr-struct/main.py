...
if (int(num_sel)==1):
    ...
    change_cut = input("The cut off values for poly inside IDR are: 0.60 or 6 residues.\nDo you want to change it (0:No, 1:Yes)? ")
    ...
    if (int(change_cut)==1):
        ...
        min_size = input("Define new accepted minimum of residues (e.g. 6): ")
        assert(min_size.isinteger()), "Value must be bigger than 6!"
        assert(int(min_size)>6), "Value must be bigger than 6!"
    
    poly_details = poly.run_poly(path_fasta, tab_poly, int(n_aa), float(cutoff), int(min_size))
    print("IDR details ({1}) were saved to disk.".format(os.path.basename(poly_details)))
