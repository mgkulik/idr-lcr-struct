...
def valid_file(filename, path):
    ''' Make sure all file names start with the proteome ID. '''
...
def get_pdb_directory(comp_path):
    ''' Search or create the pdb directory in the main project folder. '''
...
def get_pdbOut_names(files, end_name, message):
    ''' Search for the DSSP and masked PDB outputs generated by previous executions
    of the process. '''
    path=""
    if any(s.endswith(end_name) for s in files):
        path = sorted((f for f in files if f.endswith(end_name)))[-1]
        print("The following file was located in the disk: {0}".format(path))
    else:
        print(message)
    return path
