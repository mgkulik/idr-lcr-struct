...
def remove_obsoletes(ss_obsolete, path_ss_file, path_new_ss):
...
def main(comp_path):
    ...
    path_seqres = os.path.join(comp_path, date_start+"_pdb_seqres.txt")
    ...
    path_ss_final = os.path.join(comp_path, date_start+"_pdb_ss_final.txt")
    remove_obsoletes(obsolete_names, path_ss_file, path_ss_final)
    ...
    if len(missing_ids)>0:
        ...
        file_name = os.path.join(comp_path, date_start+"_pdb_ss.pickle")
        resources.save_pickle(ss2append, file_name, 'wb')
        append_ss_tofasta(ss2append, path_ss_final)
        file_error = os.path.join(comp_path, date_start+"_download_error.txt")
        resources.save_sep_llists(errors_dssp, file_error, "\t")
        path_masked_final = os.path.join(comp_path, date_start+"_pdb_masked.txt")
        generate_masked_fasta(path_ss_final, path_masked_final, seqres_desc)
        #path_masked_pkl = os.path.join(comp_path, date_start+"_pdb_masked.pickle")
